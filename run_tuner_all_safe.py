"""
Safer tuner for all datasets: uses smaller k choices and conservative hyperparams
to avoid OOM and to try to find models with mean CV accuracy inside 0.80 - 0.98.
"""
import sys
sys.path.insert(0, '.')
...
def try_tune_safe(ds_key='cicids', sample_size=10000, cv_folds=5, target_min=0.80, target_max=0.98):
    print('Loading dataset...', ds_key, sample_size)
...
if __name__ == '__main__':
    ...
    args = p.parse_args()
    res = try_tune_safe(args.ds, sample_size=args.n, cv_folds=args.cv)
    print('Result:', res)
